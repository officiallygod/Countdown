...
(function () {
  ...
  function renderHolidays(list) {
    ...
    for (const item of list) {
      ...
      left.innerHTML = `<strong>${item.date}</strong> <span class="label">${item.label ? '— ' + escapeHtml(item.label) : ''}</span>`;
      const btn = document.createElement('button');
      btn.textContent = 'Remove';
      btn.addEventListener('click', () => {
        const next = loadHolidays().filter(x => x.date !== item.date || x.label !== item.label);
        saveHolidays(next);
        renderHolidays(next);
        recompute();
      });
